...
A SwiftUI travel guide app: 23 days along the route London → Paris → Bruges → Beaune → Annecy → Chamonix → Les Arcs 1950 → Aosta Valley → Lyon, September 2026.
...
- **102 offline routes** — walking, driving, and Eurostar through the Channel Tunnel
...
| Geometry | Google Encoded Polyline (102 routes in `BundledRoutes.swift`) |
...
├── BundledRoutes.swift      # 102 offline routes (auto-generated)
...
All 102 routes are pre-fetched and bundled in `BundledRoutes.swift` — the app renders real road geometry with no network required. To refresh routes:
...
17 tests covering data integrity for all 23 days, offline geometry presence for all 102 routes, and coordinate accuracy.
