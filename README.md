...
1. Откройте `TripGuide.xcodeproj` в Xcode 16+
2. Выберите target `TripGuide`, минимальная платформа iOS 17.0
3. Подключите iPhone или iPad, нажмите ▶ Run
4. При первом запуске на устройстве: **Настройки → Основные → VPN и управление устройством** → доверять профилю

Для постоянной установки без Xcode — TestFlight с платным Apple Developer аккаунтом ($99/год).
